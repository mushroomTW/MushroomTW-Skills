...
### Quality Gate (a third-party package is adopted only when all five pass)
...
> A standard library adds no dependency, so the maintenance-commit, dependency-tree, and bundle-size thresholds do not apply; confirm instead that the target version ships it, the license is compatible, and the API fits.

### Decision templates

```
Decision: used <pkg>@<ver> because gate 1-5 pass, <reason>.
...
|---|---|---|---|---|
| Exponential backoff retry (Node) | `exponential backoff retry` | `cockatiel`/`p-retry` + standard library | `cockatiel` 1-5 pass | `Decision: used cockatiel@<ver> because gate 1-5 pass, non-invasive API and light dependencies` |
...
- Agent definition: `agents/openai.yaml` (same directory as this SKILL.md).
