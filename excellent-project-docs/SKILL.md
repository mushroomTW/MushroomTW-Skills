...
- **Visual elements**: the badges the evidence supports, each with its dynamic endpoint or recorded source ([readme-framework](references/readme-framework.md)), and the logo, screenshots, or demo assets already in the repository. Ask whether to include any, which ones, and which shields.io style; offer to keep an existing style.
