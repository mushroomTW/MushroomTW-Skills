...
- **Presentation level**: **plain** (the default: text, tables, code, and whatever badges or images the user picks below) or **showcase** (a designed page in one palette: a header block with a skill-drawn banner and a facts line, themed Mermaid diagrams, a feature-card grid, collapsed reference tables, panel-style section breaks — the recipe in [visual-readme](references/visual-readme.md)). Offer showcase as an option; do not recommend it. When offering it, include the style proposal the recipe derives from the project — accent, mood, motif — with one alternative, so the user picks a look rather than answering an open question.
