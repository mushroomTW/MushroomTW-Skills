...
- Contrast: text against the colour directly behind it reaches a WCAG contrast ratio of at least 4.5:1; the 3:1 allowance for large text applies only from 32 units regular or 25 units bold, because a 1200-unit canvas renders at about 900 pixels and WCAG's large-text sizes are 24 px regular and 18.66 px bold. The ground is what actually renders under the glyphs: a card's fill, or for a gradient or glow the interpolated colour at the text's position — take it from the rendered PNG (sample a pixel beside the text) or interpolate the stops at that offset; never the nearest stop. Compute the ratio with the relative-luminance formula (a few lines of Python) for each text colour and its ground; adjust the ground, the text colour, or the glow until every pair passes. Grey captions are where this fails first: `#7a7a7a` on `#0f0f0f` is 4.47:1 and fails for 14-unit text; `#8a8a8a` passes at 5.55:1.
