...
- Contrast: at least 4.5:1 (WCAG relative-luminance formula, a few lines of Python) between each text and the colour actually rendered under it — a card's fill, or on a gradient or glow the colour at the text's position, sampled from the rendered PNG at the darkest and lightest points along the line of text; never the nearest stop. The 3:1 large-text allowance starts at 32 units regular or 25 units at weight 700 or heavier (WCAG's 24 px and 18.66 px at 0.75 px per unit). A failing pair is fixed by changing the text colour, the ground, or the glow, then rendered again. Grey captions fail first: `#7a7a7a` on `#0f0f0f` is 4.47:1; `#8a8a8a` passes at 5.55:1.
