...
- **Animation** is out: a README banner is read, not watched, and a moving banner says nothing a static one does not. The same holds for every hand-drawn figure — see [Rules for every figure](#rules-for-every-figure) — and a `prefers-reduced-motion` fallback is not an exception, because the animation itself is never written.
...
- The `aria-label` still matches the figure: read it as the only thing a screen-reader user gets, and if any fact on the canvas is missing from the label, the label is wrong, not the figure.
...
- A hand-drawn SVG carries an accessible name: `role="img"` on the root and one `aria-label` sentence that states every fact the figure shows, so a reader who cannot see the image still gets what the prose beside it gives. A `<title>` as the first child is a useful addition, not a substitute, when the renderer shows tooltips.
- No hand-drawn SVG animates. CSS `@keyframes`, SMIL `<animate>`, and a `prefers-reduced-motion` guard that would disable them are all out: a figure is read beside prose, not watched, and `validate_docs.py` flags any of them.

### SVG composition grammar

The rules above fix what a figure must be true of; this is how the panels of a designed figure are put together so the page reads as one system rather than a pile of boxes. Apply it to hand-drawn SVG in `assets/`, under the same palette and the same richness floor — it is presentation, not a second evidence tier.

**Structure.** Open the drawing with a `<style>` block that names the recurring text roles once — a `.mono` class for identifiers and edge labels, a `.muted` class for captions, a `.label` class for section eyebrows — and set the root font-family there, instead of repeating the same attributes on every `<text>`. Put shared geometry in `<defs>`: one arrow `marker` per role colour, so an edge's arrowhead always matches its stroke, and — when the domain cue asks for texture — a small dot-grid `<pattern>` used at low opacity on the ground. Corner registration crosshairs (a cross of two 1-unit strokes inset from each corner) are decoration only; they carry no fact and never substitute for a label.

**Cards and panels.** A solid border in the accent, on a light fill, marks a step the reader must take; a dashed border marks something optional, deferred, or still keep-working. Status that the reader needs at a glance (pass, fail, optional) is a small square in the card's top-right corner in the role's colour — not a second badge row. Edge labels sit above their edge in `.mono`, coloured the same as the stroke, so the label and the flow it names cannot drift apart visually. When one figure has clearly separated zones (an agent loop beside a human-review lane, say), open each zone with a numbered band — `01 · AGENT LOOP` letter-spaced in `.label`, with a one-line `.muted` sub underneath — so the bands, not prose, carry the split. A figure may end with a takeaway row of at most three cells, each one a fact already counted in the evidence inventory (a duration, a count, a guarantee) with its short label above; three is the ceiling because the row competes with the prose beside the figure.

**Pixel motif — only when the domain owns it.** Games, terminals, hex editors, and other retro-adjacent tools have a shape language their readers recognise before they read the name. When the domain cue points there, three primitives are available; when it does not, they stay out of the proposal:

- Set `shape-rendering="crispEdges"` on the root and draw on an integer grid, so edges land on pixel boundaries at any scale.
- Build frames as two or three nested stepped paths (the outermost darkest, the middle the border colour), not as a single `stroke` on a rounded rect. The step unit stays a whole number on the same grid.
- Draw short labels as a grid of 1- or 2-unit `<rect>`s when the label *is* the artefact (a pixel wordmark, a badge legend); otherwise use normal text. Either way the effective size still clears the 12-unit legibility floor — measure the glyph grid against the canvas, not the rect unit alone. Depth, when needed, is a duplicate of the glyph rects offset by (1,1) or (3,3) in a darker tone, never a filter or a gradient on text.

Block meters (a row of equal `<rect>`s, filled or empty, for a count or ratio) fall under the same gate. The number itself still lives in the facts line or the prose beside the figure — a meter that is the only home of a count violates the single-copy rule the moment the renderer drops the image.

This motif is a shape vocabulary inside the four banner directions and the figure recipes, not a fifth direction: it changes how a terminal mockup's chrome is drawn, not which direction was chosen.

**Accessible by construction.** Every hand-drawn SVG — banner or figure — carries `role="img"` on the `<svg>` root and an `aria-label` written as one sentence that names the figure's subject and the facts it displays, in the README's language. The label is the figure's alt text for a screen reader and must stay true when the image fails to load, so it restates what the prose already says rather than inventing a caption. A `<title>` element as the first child is optional; add it when a tooltip on hover helps, and keep it consistent with the `aria-label`.
...
- Animates a banner or figure, with or without a `prefers-reduced-motion` guard: the animation is never written, so there is nothing for the guard to disable.
- Copies a third-party logo, wordmark, screenshot, or font into a drawing — recognisable or not, if it is not a file the repository already carries, it does not go in.
