...
Apply **Reader and content** through **Writing quality** to every document you touched, reading each one as its own reader; apply **Showcase** only when the user chose it; apply **Document set** once to the whole set.
...
- [ ] A hand-drawn badge row is all hand-drawn (no mixed shields.io styles), every value has a `<!-- badge source: … -->` note, and no badge carries a license.
...
- [ ] All badges share one badge style — a single shields.io style or a fully hand-drawn row — and read as a single row.
...
- [ ] The style (accent, mood, domain cue, banner direction) was proposed from the project's own assets, artefacts, and concepts and chosen by the user at the checkpoint.
- [ ] The rendered banner was shown to the user as a PNG and accepted before the page was drafted.
- [ ] One palette runs through banner, figures, and cards.
- [ ] The banner follows a direction that fits the project, carries the name as its heaviest element, and carries a figure, mockup, or fact set that fails the swap test.
- [ ] A domain reader recognises one element at a glance, and it is drawn — never a copied logo or asset.
- [ ] A mockup shows only commands and output the program actually emits, each line with its provenance in the report.
- [ ] Every figure — Mermaid themed to the palette or hand-drawn SVG — meets the richness floor (four nodes plus an edge label or region) and maps every node to an evidence-inventory entry.
- [ ] Every banner and card fact was counted or read from the repository, and the prose beside each visual stands on its own.
- [ ] Every hand-drawn SVG carries `role="img"` and an `aria-label` that restates the facts it shows, loads nothing from outside the repository, and contains no CSS or SMIL animation.
- [ ] The composition follows the grammar in visual-readme: shared style classes, role-coloured markers, card grammar, and the pixel motif only when the domain cue asks for it.
- [ ] Every SVG has an opaque rect covering the whole canvas, and `xml:space="preserve"` on text that aligns on spaces.
- [ ] Text is fitted by the width rules: Latin lines that must not overflow are pinned with `textLength`; CJK lines are sized by arithmetic and never pinned with `spacingAndGlyphs`.
- [ ] The banner was rendered at the desktop and phone container widths and over a dark page, or the arithmetic check is listed under **Unrun checks**.
- [ ] The rendered README was opened on github.com once when the change was already pushed, or the GitHub-side render is listed under **Unrun checks**.
