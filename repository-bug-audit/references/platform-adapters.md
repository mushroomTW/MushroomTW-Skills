...
- Stay read-only: only the `.docs/` report and its transient evidence file; only run repo-configured checks. No tool install / repro probes.
- Multi-agent: one subagent per partition (inventory, flows, candidates, limitations); cross-review High without revealing original verdict.
