...
This repository ships the skill as a plain directory. Clone it once, then copy `skills/excellent-readme/` into the skill directory of each product.
...
The `Remove-Item` line makes the same commands work for a reinstall: without it, `Copy-Item` nests a second copy inside the existing skill directory. To install the skill for one repository instead of the whole account, use that repository's `.claude/skills/` (Claude Code) or `.agents/skills/` (Codex) as the destination. Both products read their skill directories when a session starts, so start a new session after copying.

> [!IMPORTANT]
> The clone requires read access to the GitHub repository. Configure the appropriate Git credentials first if the repository is private.

## Canonical Skill Location

The only skill entry point is [`SKILL.md`](skills/excellent-readme/SKILL.md). Its supporting files stay beside it in the same directory:
...
The complete workflow is in [`SKILL.md`](skills/excellent-readme/SKILL.md). Its README framework, delivery checklist, and style exemplars are in the skill's [`references/`](skills/excellent-readme/references/) directory.
...
```powershell
python skills/excellent-readme/scripts/validate_readme.py README.md --project .
```
