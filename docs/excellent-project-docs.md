...
- Offers two presentation levels: plain (default) or showcase, which turns the same facts into a designed page in one palette: a header block with a skill-drawn poster-style SVG banner (eyebrow, wordmark, flow line, and a data motif such as stacked tiers or a bar ramp) and a facts line, themed Mermaid diagrams, a feature-card grid, and collapsed reference tables — every shape and number drawn from the evidence.
