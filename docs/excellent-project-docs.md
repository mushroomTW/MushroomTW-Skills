...
- Offers two presentation levels: plain (default) or showcase, which turns the same facts into a designed page in one palette: a header block with an SVG banner the skill designs for the project (editorial poster, terminal mockup, figure-first, or split panel) and a facts line, one figure per concept as themed Mermaid or hand-drawn SVG with a shared composition grammar (role-coloured markers, card grammar, pixel motif only when the domain owns it, and `role="img"` plus an `aria-label` on every hand-drawn SVG), a feature-card grid, and collapsed reference tables — every shape, number, and output line drawn from the evidence. Badge rows may use shields.io styles or a fully hand-drawn static SVG badge when the value has a recorded source and the project's identity needs it.
...
3. Select the README sections and the companion documents the evidence justifies; propose any new companion file, ask the presentation questions (plain or showcase level with a style proposal derived from the project — accent, mood, domain cue, banner direction — badges and images, badge style including an optional hand-drawn static SVG row when a dynamic endpoint does not exist, leading install channel, tagline), and wait for approval. The audience and language questions from step 1 go in the same message, so every pre-draft question arrives at once; with no answer, a new README is written in English alone and the choice is reported as assumed. In showcase the banner is drafted and rendered first, and its PNG is approved before the page is written.
